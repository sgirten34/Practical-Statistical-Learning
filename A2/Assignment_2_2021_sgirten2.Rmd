...
```{r test function, message=FALSE, warning=FALSE}
...
```{r test vs glmnet, message=FALSE, warning=FALSE}
library(glmnet)
...
The Lasso.1se and L.Refit models have the poorest performance for MSPE.  
...
Refitting does not seem to be advantageous in this case.  There is a very little difference in performance for the refitted model with approximately the same mean performance. Generally the refitted model's performance is in line with the Lasso.1se performance.  
...
*Which procedure or procedures yield the best performance in terms of MSPE?*  
The Lasso.min and L.Refit models produced the best performance in terms of MSPE.  

*Conversely, which procedure or procedures show the poorest performance?*  
The PCR and Ridge.min models showed the poorest performance for MSPE.  
...
PCR and Ridge.min models had roughly the same performance as the Lasso and Full models in Case I, but exhibited poorer performance than the Lasso models in Case II.  Since Case II introduces an additional 500 features of artificial noise, it seems the Lasso models are able to set the value for most of the 500 noise features to 0 and eliminate those features from the model while the PCR and Ridge.min models still incorporate the noise features into their models and thus generate poorer performance due to more noise being present in the model itself.  
...
No, my results do not corroborate this expectation - my results showed a higher MSPE for Case II. The addition of 500 extra features of noise likely caused the decrease in MSPE performance.  Even though Lasso and Ridge models try to limit the effect of non-significant features in the model, the additional noise in the data seems to still have some impact on the model and is causing the decrease in performance.  
